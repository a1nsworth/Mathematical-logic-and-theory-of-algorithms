...
namespace Laba1
{
    public interface IOperationsBehavior
    {
        ...
        bool IsRequiredCharacter(char c);
    }
    ...
    public abstract class Operations
    {
        ...
        protected bool IsRequiredCharacter(char c) => _operationsBehavior.IsRequiredCharacter(c);
    }
    ...
    public class Сalculator<T> : Operations
    {
        ...
        public Сalculator(string expression) =>
            ValueExpression = ValueExpression = CalculatingExpression(GetOutputReversePolishNotation(expression));
        ...
        private T CalculatingExpression(in string expression)
        {
            ...
            foreach (var symbol in expression)
            {
                resultStack.Push(IsRequiredCharacter(symbol)
                    ? PerformConvertValue<T>(symbol)
                    : PerformOperationBehavior(ref resultStack, symbol));
            }
